...
const addFriend = async (req, res) => {
  ...
  try {
    let loggedInUser = await User.findById(req.params.id).populate(
      "friends",
      "_id username"
    );
    ...
    if (user && loggedInUser) {
      if (
        (loggedInUser.friends.length !== 0 &&
          loggedInUser.friends.findIndex((friend) =>
            friend._id.equals(user._id)
          ) !== -1) ||
        (user.friends.length !== 0 &&
          user.friends.findIndex((friend) =>
            friend._id.equals(loggedInUser._id)
          ) !== -1)
      ) {
...
const deleteFriend = async (req, res) => {
  ...
  try {
    ...
    if (user && loggedInUser) {
      loggedInUser.friends.splice(
        loggedInUser.friends.findIndex((friend) => friend._id.equals(user._id)),
        1
      );
      loggedInUser.save();
      user.friends.splice(
        user.friends.findIndex((friend) => friend._id.equals(user._id)),
        1
      );
      user.save();
      res.status(200).json(loggedInUser);
